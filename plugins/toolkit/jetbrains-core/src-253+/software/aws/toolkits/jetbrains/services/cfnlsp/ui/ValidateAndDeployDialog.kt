...
internal class ValidateAndDeployWizard(
    ...
    private val configStep = ConfigurationStep(
        project, prefilledTemplatePath, prefilledStackName,
...
private class ConfigurationStep(
    ...
    private val component = panel {
        group("Template & Stack") {
            row(message("cloudformation.deploy.dialog.template.label")) { cell(templateField).align(Align.FILL) }
            row(message("cloudformation.deploy.dialog.stack_name.label")) { cell(stackNameField).align(Align.FILL) }
        }
    ...
    fun validate(): String? {
        ...
        if (path.isBlank()) return message("cloudformation.deploy.dialog.template.required")
        val file = File(path)
    ...
    fun saveState() {
        ...
        state.lastTemplatePath = templateField.text.trim()
    ...
    fun getTemplatePath(): String = templateField.text.trim()
...
// Step 2: Import resources — select resources and enter identifiers
    ...
    private val component = JBPanel<JBPanel<*>>(BorderLayout()).apply {
        ...
        val bottomPanel = panel {
            group("Resource identifiers") {
                row {
                    cell(JBScrollPane(identifierTable).apply {
